...
Na sequência, analisamos a distribuição das variáveis contínuas dentro de cada
uma das categorias de qualidade observadas. Para entender o nosso propósito,
considere qualquer uma dessas variáveis. O objetivo é responder às seguintes
perguntas: entre os vinhos de péssima qualidade, como é a distribuição dessa
variável? E entre os melhores vinhos?

A resposta para essas questões pode ser visualizada através de uma série de
boxplots.

```{r boxplot_qualidade_funcao}
## Função para criar os boxplots que consideram a qualidade dos vinhos. Além
## dos dados, esta função recebe 2 strings, o nome da coluna e o rótulo do eixo
## y.
cria_boxplot_qualidade <- function(vinhos, coluna, rotulo) {
...
```{r boxplot_qualidade_fixed_acidity}
cria_boxplot_qualidade(vinhos, "fixed.acidity", "Fixed Acidity")
```

**Boxplot da variável na coluna `volatile.acidity`:**

```{r boxplot_qualidade_volatile_acidity}
cria_boxplot_qualidade(vinhos, "volatile.acidity", "Volatile Acidity")
```

**Boxplot da variável na coluna `citric.acid`:**

```{r boxplot_qualidade_citric_acid}
cria_boxplot_qualidade(vinhos, "citric.acid", "Citric Acid")
```

**Boxplot da variável na coluna `residual.sugar`:**

```{r boxplot_qualidade_residual_sugar}
cria_boxplot_qualidade(vinhos, "residual.sugar", "Residual Sugar")
```

**Boxplot da variável na coluna `chlorides`:**

```{r boxplot_qualidade_chlorides}
cria_boxplot_qualidade(vinhos, "chlorides", "Chlorides")
```

**Boxplot da variável na coluna `free.sulfur.dioxide`:**

```{r boxplot_qualidade_free_sulfur_dioxide}
cria_boxplot_qualidade(vinhos, "free.sulfur.dioxide", "Free Sulfur Dioxide")
```

**Boxplot da variável na coluna `total.sulfur.dioxide`:**

```{r boxplot_qualidade_total_sulfur_dioxide}
cria_boxplot_qualidade(vinhos, "total.sulfur.dioxide", "Total Sulfur Dioxide")
```

**Boxplot da variável na coluna `density`:**

```{r boxplot_qualidade_density}
cria_boxplot_qualidade(vinhos, "density", "Density")
```

**Boxplot da variável na coluna `ph`:**

```{r boxplot_qualidade_ph}
cria_boxplot_qualidade(vinhos, "ph", "pH")
```

**Boxplot da variável na coluna `sulphates`:**

```{r boxplot_qualidade_sulphates}
cria_boxplot_qualidade(vinhos, "sulphates", "Sulphates")
```

**Boxplot da variável na coluna `alcohol`:**

```{r boxplot_qualidade_alcohol}
cria_boxplot_qualidade(vinhos, "alcohol", "Alcohol")
```

...
cria_boxplot <- function(vinhos, coluna, rotulo) {
  ...
  ## Seleciona apenas as colunas que importam neste caso. Traduz o nome da
  ## coluna `type`.
  vinhos_2_colunas <-
    vinhos %>%
      select(c("type", coluna)) %>%
      rename(Tipo = type)

  ## Traduz os nomes dos tipos dos vinhos
  vinhos_2_colunas$Tipo <- recode(
    vinhos_2_colunas$Tipo,
    "red" = "Tintos",
    "white" = "Brancos"
  )

  ## Usando o ggplot2, cria o boxplot
  ggplot(vinhos_2_colunas, aes_string(y = colnames(vinhos_2_colunas)[2])) +
    geom_boxplot() +
    labs(y = rotulo) +
